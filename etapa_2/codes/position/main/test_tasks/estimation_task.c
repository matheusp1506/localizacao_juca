...
portTASK_FUNCTION(PosEstimation, args) {
    ...
    while (1) {
        if (xQueueReceive(xRobotDataQueue, &local_data, portMAX_DELAY) == pdPASS) {
			...
			// Converte o quaternion atual para radianos
			float current_theta = 2.0f * atan2f(local_data.quat_z, local_data.quat_w);
			...
			float delta_x = delta_s * cosf(last_theta + (delta_theta / 2.0f));
			float delta_y = delta_s * sinf(last_theta + (delta_theta / 2.0f));
			...
			ESP_LOGI(TAG, "Pose Estimada: x=%.3f m, y=%.3f m, theta=%.3f rad", x, y, current_theta);
