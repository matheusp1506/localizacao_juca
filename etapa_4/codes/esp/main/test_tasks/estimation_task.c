    ...
    while (1) {
        if (xQueueReceive(xRobotDataQueue, &local_data, portMAX_DELAY) == pdPASS) {
			...
			*/
			...
			float integration_angle = theta_enc + (delta_theta / 2.0f);

			float delta_x = delta_s * cosf(integration_angle);
			float delta_y = delta_s * sinf(integration_angle);
			x = x + delta_x;
			y = y + delta_y;
	        ...
	        msg_to_send.x = x;
	        msg_to_send.y = y;
			msg_to_send.pulses_left = local_data.pulses_left;
			msg_to_send.pulses_right = local_data.pulses_right;
