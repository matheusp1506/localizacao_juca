...
void test1(){
	while(1){
		wheel_GoForward();
		wheel_SetVel(BASE_SPEED, BASE_SPEED);
...
portTASK_FUNCTION(wheel_ctrl, arg)
{
	...
	vTaskDelay(pdMS_TO_TICKS(5000)); // espera 5 segundos para iniciar os testes, para dar tempo de subir o monitor serial e tudo mais
	wheel_GoForward();
	wheel_SetVel(BASE_SPEED, BASE_SPEED);
